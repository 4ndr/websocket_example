"""DriveAPI URL Configuration

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/1.10/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  url(r'^$', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  url(r'^$', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.conf.urls import url, include
    2. Add a URL to urlpatterns:  url(r'^blog/', include('blog.urls'))
"""
import rinzler
from rinzler.core.main_controller import MainController

from onyxerp.core.services.jwt_service import JwtService

from websockets_test.controllers.data_controller_v2 import DataControllerV2
from websockets_test.settings import CONFIG

app = rinzler.boot("MetaAPI")

jwt_service = JwtService(CONFIG, app)
assembler = app['url_assembler']
assembler.set_auth_service(jwt_service)

urlpatterns = [
    assembler.mount('chat', DataControllerV2),
    assembler.mount('', MainController)
]
